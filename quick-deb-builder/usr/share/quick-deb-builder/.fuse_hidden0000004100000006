...
create_deb_package()
{
	...
	echo "${executable_files[*]}" | xargs chmod 755 # Dá permissões rwxr-xr-x para todos os arquivos executáveis
	echo "${non_executable_files[*]}" | xargs chmod 644 # Dá permissões rw-r--r-- para todos os arquivos não-executáveis
	chmod 0755 DEBIAN/ || chmod 0755 debian/; # Dá permissões rwxr-xr-x para pasta debian

	sudo chown -R root: /tmp/deb-packing; # Não vai pedir senha por causa do "sudoers.d"
	dpkg-deb -b /tmp/deb-packing "${PACKAGE_PATHS[1]}";
}
